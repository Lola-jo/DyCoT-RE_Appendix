...
While these benchmarks are widely used, recent studies have raised concerns that large language models may possess latent knowledge about 
...
Recent frameworks exemplify the growing use of LLMs for automated reward engineering in RL.
EUREKA \cite{ma2023eureka} treats LLMs as autonomous agents that iteratively generate, mutate, and select reward code via performance-based tournaments.
Text2Reward \cite{xie2023text2reward} focuses on direct reward code generation from natural language, while CARD \cite{sun2024large} emphasizes iterative code refinement guided by LLM feedback.

Beyond static reward generation, recent feedback-driven approaches such as adversarial prompt detection [Xie et al., 2024], self-play alignment [Wu et al., 2024], and benchmarking frameworks [Song et al., 2025] have advanced reward modeling quality. 
...
intent and reinforcement learning objectives will be critical for the next generation of intelligent systems. 
...
Recent studies have systematically explored adaptive temperature mechanisms. 
Zhu et al. \cite{zhu2024hot} proposed AdapT, an adaptive sampling strategy for code generation tasks, 
which dynamically adjusts decoding temperature based on token-level difficulty to improve generation quality. 
Zhang et al. \cite{zhang2024edt} introduced Entropy-based Dynamic Temperature (EDT) sampling, 
which regulates output entropy and diversity in natural language generation. 
Cecere et al. \cite{cecere2025monte} proposed Monte Carlo Temperature, 
a robust sampling method to enhance uncertainty quantification under distribution shifts. 
Chang et al. \cite{chang2023kl} leveraged KL-divergence-guided temperature control to modulate exploration adaptively.

Beyond static sampling strategies, recent work has begun addressing the broader challenge 
of adapting reward generation mechanisms to evolving training dynamics. 
Evstafev \cite{evstafev2025paradox} highlighted limitations of stochastic sampling in tasks requiring structured outputs, 
motivating the need for more context-aware adaptation techniques. 
Nakaishi et al. \cite{nakaishi2024critical} further revealed phase-transition behaviors in LLM sampling regimes, 
informing principled temperature scaling strategies.

For model selection, various strategies have been proposed to enhance efficiency, robustness, and alignment under resource constraints. 
Zhou et al. \cite{zhou2022mixture} introduced expert choice routing to improve Mixture-of-Experts (MoE) efficiency, 
while Li et al. \cite{li2025llm} developed preference-conditioned dynamic routing for cost-effective LLM generation. 
Hu et al. \cite{hu2024dynamic} presented Dynamic Ensemble Reasoning to integrate outputs from multiple specialized LLMs, enhancing robustness. 
Li et al. \cite{li2025revisiting} revisited self-consistency decoding from a distributional alignment perspective, 
offering insights into stable expert aggregation.

Despite recent advances, temperature adaptation has mainly aimed at enhancing generative diversity and calibration, 
...
a custom environment with multi-objective resource collection, sparse rewards, 
and dynamic energy constraints.
...
We evaluate DyCoT-RE on three environments from the Gymnasium\footnote{\url{https://gymnasium.farama.org}} suite—CartPole-v1\footnote{\url{https://gymnasium.farama.org/environments/classic_control/cart_pole/}}, BipedalWalker-v3\footnote{\url{https://gymnasium.farama.org/environments/box2d/bipedal_walker/}}, and Ant-v5\footnote{\url{https://gymnasium.farama.org/environments/mujoco/ant/}}—alongside a custom-designed SpaceMining environment built to test generalization in unseen domains.
...
0.2 & 242.74 & \cellcolor{grass!25}313.60/-20.27 & 125.47 & 1.93 \\
0.3 & 210.07 & 311.17/-46.69 & 140.49 & 1.50 \\
0.4 & 244.19 & 311.94/-17.13 & {grass!25}107.02 & \cellcolor{grass!25}2.28 \\
0.5 & 215.84 & 310.69/-14.25 & 131.01 & 1.65 \\
0.6 & 248.11 & 312.48/-21.18 & \cellcolor{grass!15}120.45 & \cellcolor{grass!15}2.06 \\
0.7 & 225.88 & \cellcolor{grass!15}312.42/-12.08 & 131.43 & 1.72 \\
...
Figure~\ref{fig:space_mining_curves} illustrates the learning trajectory of DyCoT-RE. From early-stage exploration to late-stage optimization, we observe concurrent increases in reward and fitness, with consistent energy regulation and inventory accumulation. These trends reflect reward structures that incentivize planning, efficiency, and safe behavior.

\begin{table}[htbp]
\centering
\caption{Core Metrics Evolution Across Training}
\label{tab:metrics_evolution}
\begin{tabular}{lcccc}
\toprule
Metric & Initial & Final & Corr. & Key Pattern \\
\midrule
Raw Reward & 12,400 & 107,882 & 1.00 & Exponential growth \\
Energy Level & 150 & 32 & -0.84 & Linear depletion \\
Inventory & 0 & 78 & +0.99 & Logistic growth \\
Collisions & 1.2 & 4.7 & -0.75 & Quadratic rise \\
Fitness Score & 0.15 & 0.92 & -0.32 & S-curve \\
\bottomrule
\end{tabular}
\end{table}

As shown in Table~\ref{tab:metrics_evolution}, DyCoT-RE refines its reward formulation over iterations. Initial designs focused on spatial feedback, but later stages incorporated delivery efficiency, energy conservation, and motion regularization. The reward weighting changes reflect adaptive emphasis: e.g., delivery gains from 0.0 to 2.0 between Iter 1--3, while energy dominates by Iter 7 due to emerging resource bottlenecks.

\begin{figure}[ht]
\centering
\includegraphics[width=0.6\linewidth]{Figures/correlation_heatmap.pdf}
\caption{
Correlation among key variables during SpaceMining training. Reward strongly correlates with inventory and fitness, and inversely with energy. Weak correlation with collisions suggests a tolerance of moderate risk to sustain output.
}
\label{fig:space_mining_corr}
\end{figure}

Figure~\ref{fig:space_mining_corr} presents correlations among core variables during training. Reward shows near-linear correlation with both inventory and fitness, highlighting resource collection as the main driver of performance. Its negative association with energy indicates an underlying efficiency constraint. The weak correlation with collisions suggests that moderate risk is acceptable if it increases overall throughput.

To support such evolution, DyCoT-RE employs a dual-dynamic mechanism combining temperature adjustment and confidence stabilization. Table~\ref{tab:dycot_dynamics} summarizes how these variables co-evolve with reward quality.

\begin{table}[htbp]
\center
\scriptsize
\caption{Dual-Dynamic Mechanism Evolution in SpaceMining. 
Data shown for selected odd-numbered iterations to illustrate meaningful dynamics across training.}
\label{tab:dycot_dynamics}
\begin{tabular}{c|c|c|c|c}
\toprule
Iter & Temp & Entropy & Confidence & Best Reward \\
\midrule
1 & 0.700 & 0.946 & 0.946 & 1124.4 \\
3 & 0.639 & 0.986 & 0.878 & 3475.5 \\
5 & 0.556 & 0.914 & 0.807 & 3801.3 \\
7 & 0.517 & 0.972 & 0.824 & 4003.2 \\
\bottomrule
\end{tabular}
\end{table}

Lower temperature leads to more deterministic reward generation, while entropy and confidence guide semantic precision. Iteration 5 introduced velocity normalization, reducing erratic exploration, and Iteration 7 emphasized energy awareness, aligning model output with strategic planning needs.

\begin{table}[htbp]
\centering
\scriptsize
\caption{Performance Impact of Dynamic Adjustments. 
Reported transitions correspond to major updates in reward and strategy; intermediate iterations omitted for clarity.}
\label{tab:impact_analysis}
\begin{tabular}{c|c|c|c|l}
\toprule
Transition & $\Delta$Temp & $\Delta$Entropy & $\Delta$Conf. & Impact \\
\midrule
1 $\rightarrow$ 3 & $-$8.7\% & +4.3\% & $-$7.2\% & +31.2\% reward (delivery boost) \\
3 $\rightarrow$ 5 & $-$13.0\% & $-$7.3\% & $-$8.0\% & +42.4\% reward (speed tuning) \\
5 $\rightarrow$ 7 & $-$7.0\% & +6.4\% & +2.1\% & +4.8\% reward (energy efficiency) \\
\bottomrule
\end{tabular}
\end{table}

Table~\ref{tab:impact_analysis} quantifies how dynamic parameter shifts correlate with policy improvement. Notably, the largest jump occurs between Iter 3 and 5, where reward restructuring and confidence realignment yield over 42\% gain. Later refinements further stabilize performance, reducing collisions by 41\% and improving energy discipline.
...
Future directions include integrating DyCoT-RE with offline reinforcement learning to reduce the reliance on online sampling, extending model selection to multimodal architectures for vision-language tasks, and incorporating human-in-the-loop evaluation to refine reward quality assessment beyond component-level decomposition.
...
This paper proposed DyCoT-RE, a reward engineering framework for reinforcement learning that combines chain-of-thought reasoning with dual-dynamic optimization. By integrating structured reasoning, dynamic temperature adjustment, and model selection across training iterations, DyCoT-RE automates the generation and refinement of reward functions using large language models.  

Experimental results across four environments—including discrete control, continuous locomotion, and resource-centered tasks—demonstrate that DyCoT-RE leads to faster convergence and improved policy performance compared to manually designed and single-strategy baselines. Its ability to adapt reward structures over time highlights the potential of language model–driven reasoning to support general-purpose reward engineering in knowledge-sparse settings.
