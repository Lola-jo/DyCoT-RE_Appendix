...
Unlike standard benchmarks, the environment provides no predefined reward signals or task-specific design priors. The agent observes partial information about its position, energy, and surrounding asteroids (total 53D), and must produce continuous thrust and mining actions to collect resources efficiently. This setup explicitly tests whether LLM-based reward design can succeed through pure task and code understanding, without prior dataset bias.
...
The baselines include manually specified reward functions from standard Gymnasium implementations such as CartPole, BipedalWalker, and Ant, along with custom rules tailored for the SpaceMining environment. These represent conventional RL practice with expert prior knowledge.
...
We evaluate DyCoT-RE on three benchmark tasks from the Gymnasium\footnote{\url{https://gymnasium.farama.org}} suite—CartPole-v1\footnote{\url{https://gymnasium.farama.org/environments/classic_control/cart_pole/}}, BipedalWalker-v3\footnote{\url{https://gymnasium.farama.org/environments/box2d/bipedal_walker/}}, and Ant-v5\footnote{\url{https://gymnasium.farama.org/environments/mujoco/ant/}}—alongside a custom-designed SpaceMining environment built to test generalization in unseen domains.
...
\begin{tabular}{lccc}
\toprule
\textbf{Environment} & \textbf{Dimensions (S/A)} & \textbf{Steps} & \textbf{Task} \\
\midrule
...
\begin{tabular}{llccc}
\toprule
\textbf{Task} & \textbf{Method} & \textbf{Sample Eff.} $\downarrow$ & \textbf{Succ. Rate} $\uparrow$ & \textbf{Late Gain} $\uparrow$ \\
\midrule

\multirow{3}{*}{CartPole} 
& {DyCoT-RE} & \cellcolor{skyblue!15}\textbf{75k} & \cellcolor{skyblue!15}\textbf{98\%} & \cellcolor{skyblue!15}\textbf{450} \\
...
& {DyCoT-RE} & \cellcolor{skyblue!15}\textbf{393k} & \cellcolor{skyblue!15}\textbf{92\%} & \cellcolor{skyblue!15}\textbf{387} \\
...
\multirow{3}{*}{Ant} 
& {DyCoT-RE} & \cellcolor{skyblue!15}\textbf{16.19M} & \cellcolor{skyblue!15}\textbf{85\%} & \cellcolor{skyblue!15}\textbf{5398} \\
...
& {DyCoT-RE} & \cellcolor{skyblue!15}\textbf{1.31M} & \cellcolor{skyblue!15}\textbf{80\%} & \cellcolor{skyblue!15}\textbf{2285} \\
...
DyCoT-RE & \cellcolor{skyblue!25}\textbf{297.28} & \cellcolor{grass!25}\textbf{316.57} & \cellcolor{skyblue!25}\textbf{89.47} & \cellcolor{skyblue!25}\textbf{6.46} \\
\hline
\end{tabular}
\end{table}
...
\begin{tabular}{lcccc}
\toprule
\textbf{Metric} & \textbf{Initial} & \textbf{Final} & \textbf{Corr.} & \textbf{Key Pattern} \\
\midrule
...
\begin{tabular}{c|c|c|c|c}
\toprule
\textbf{Iter} & \textbf{Temp} & \textbf{Entropy} & \textbf{Confidence} & \textbf{Best Reward} \\
\midrule
...
\toprule
\textbf{Transition} & $\Delta$Temp & $\Delta$Entropy & $\Delta$Conf. & \textbf{Impact} \\
\midrule
