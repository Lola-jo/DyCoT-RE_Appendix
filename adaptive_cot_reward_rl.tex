...
Similarly, Zhang et al. developed Entropy-based Dynamic Temperature (EDT) sampling to regulate model entropy
...
as explored in Switching for Continual Instruction Tuning. GLaM leveraged Mixture-of-Experts (MoE) 
...
The dual-dynamic optimization strategy in DyCoT-RE integrates temperature adjustment and model selection into a cohesive, mutually adaptive system. Rather than operating as isolated components, these mechanisms interact to enhance reward generation quality and training stability in reinforcement learning.

At each iteration, the policy entropy $H_t$ and performance feedback $J(\theta)$ are computed to characterize agent behavior and learning progress:

\[
H_t = - \sum_{a \in \mathcal{A}} \pi_{\theta}(a|s_t) \log \pi_{\theta}(a|s_t), \quad
J(\theta) = \mathbb{E}_{\pi_\theta}\left[\sum_{k=0}^T \gamma^k R(s_k,a_k)\right].
\]

The Dynamic Temperature Adjustment module uses these signals to modulate the LLM sampling temperature $T$:

\[
T_{t+1} = T_t + \alpha \left( \frac{\partial T}{\partial H_t} \right),
\]

thereby balancing exploration diversity and generation stability.

Concurrently, the Dynamic Model Selection module chooses specialized LLMs based on module-specific performance $\hat{p}_j(f_k, z)$ and overall training improvement:

\[
k_j^* = \arg\max_{k \in M} \left[ \hat{p}_j(f_k, z) + \gamma \cdot J(\theta) \right].
\]

These two mechanisms operate in synergy: temperature adjustments influence the diversity of generated reward function candidates, shaping inputs for model selection, while model selection decisions affect reward quality and agent learning dynamics, which in turn impact entropy trends guiding temperature updates.

This coupled feedback system enables DyCoT-RE to maintain an adaptive balance between creative reward exploration and stable policy optimization, effectively leveraging LLM specialization while preserving training efficiency and interpretability.
